...
const CardSliderRec: React.FC<CardSliderRecProps> = ({ recType }) => {
  ...
  useEffect(() => {
    const fetchUserId = async () => {
      try {
        const response = await fetch("https://intexwebapp301v2-auhkc9f2htgtachk.eastus-01.azurewebsites.net/api/Users/me", {
          credentials: "include",
        });
        if (!response.ok) {
    ...
    const fetchRecommendations = async () => {
      ...
      try {
        ...
        const response = await fetch(
          `https://railway-production-e21b.up.railway.app/api/recommendations/${userId}`
        );
        if (!response.ok) {
